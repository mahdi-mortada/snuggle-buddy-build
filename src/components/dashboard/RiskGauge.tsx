...
export function RiskGauge({ score, trend }: { score: number; trend: number }) {
  const maxScore = 100;
    ...
    <div className="glass-panel h-full flex flex-col items-center justify-center p-6">
      ...
      <div className="relative">
        <svg width="200" height="160" viewBox="0 0 200 180">
          <circle cx="100" cy="100" r="80" fill="none" stroke="hsl(var(--border))" strokeWidth="12" strokeLinecap="round"
            strokeDasharray={circumference} strokeDashoffset={circumference * 0.25} transform="rotate(135 100 100)" />
          <circle cx="100" cy="100" r="80" fill="none" stroke={getColor(score)} strokeWidth="12" strokeLinecap="round"
            strokeDasharray={circumference} strokeDashoffset={offset} transform="rotate(135 100 100)"
            style={{ transition: 'stroke-dashoffset 1s ease-in-out', filter: `drop-shadow(0 0 8px ${getColor(score)}40)` }} />
        </svg>
        <div className="absolute inset-0 flex flex-col items-center justify-center pt-2">
          <span className="font-mono-data text-4xl font-bold text-foreground">{score.toFixed(1)}</span>
          <span className="text-xs font-bold uppercase tracking-widest mt-1" style={{ color: getColor(score) }}>
            {getLabel(score)}
          </span>
          <span className={`text-[10px] mt-1 font-mono-data ${trend > 0 ? 'text-critical' : 'text-success'}`}>
